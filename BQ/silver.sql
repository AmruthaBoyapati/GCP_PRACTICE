...
CREATE OR REPLACE TABLE `clear-basis-469515-u9.silver_dataset.quality_checks` AS
SELECT DISTINCT 
...
SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
from `clear-basis-469515-u9.bronze_dataset.patients_ha`
...
select distinct
         ...
         ModifiedDate,
'hosb' AS datasource
from `clear-basis-469515-u9.bronze_dataset.patients_hb`);

MERGE INTO `clear-basis-469515-u9.silver_dataset.patients` AS target
USING `clear-basis-469515-u9.silver_dataset.quality_checks` AS source
...
DROP TABLE IF EXISTS `clear-basis-469515-u9.silver_dataset.quality_checks`;

CREATE TABLE IF NOT EXISTS `clear-basis-469515-u9.silver_dataset.encounters` (
...
CREATE OR REPLACE TABLE `clear-basis-469515-u9.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `clear-basis-469515-u9.bronze_dataset.encounters_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `clear-basis-469515-u9.bronze_dataset.encounters_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `clear-basis-469515-u9.silver_dataset.encounters` AS target
USING `clear-basis-469515-u9.silver_dataset.quality_checks_encounters` AS source
...
DROP TABLE IF EXISTS `clear-basis-469515-u9.silver_dataset.quality_checks_encounters`;
CREATE TABLE IF NOT EXISTS `clear-basis-469515-u9.silver_dataset.transactions` (
...
SELECT DISTINCT 
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `clear-basis-469515-u9.bronze_dataset.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `clear-basis-469515-u9.bronze_dataset.transactions_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `clear-basis-469515-u9.silver_dataset.transactions` AS target
USING `clear-basis-469515-u9.silver_dataset.quality_checks` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `clear-basis-469515-u9.silver_dataset.quality_checks`;
CREATE TABLE IF NOT EXISTS `clear-basis-469515-u9.silver_dataset.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
TRUNCATE TABLE `clear-basis-469515-u9.silver_dataset.providers`;

-- 3. full load by Inserting merged Data 
INSERT INTO `clear-basis-469515-u9.silver_dataset.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `clear-basis-469515-u9.bronze_dataset.providers_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `clear-basis-469515-u9.bronze_dataset.providers_hb`
);


CREATE TABLE IF NOT EXISTS `clear-basis-469515-u9.silver_dataset.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `clear-basis-469515-u9.silver_dataset.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `clear-basis-469515-u9.bronze_dataset.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `clear-basis-469515-u9.silver_dataset.claims` AS target
USING `clear-basis-469515-u9.silver_dataset.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `clear-basis-469515-u9.silver_dataset.quality_checks_claims`;



 CREATE TABLE IF NOT EXISTS `clear-basis-469515-u9.silver_dataset.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `clear-basis-469515-u9.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `clear-basis-469515-u9.bronze_dataset.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `clear-basis-469515-u9.silver_dataset.cpt_codes` AS target
USING `clear-basis-469515-u9.silver_dataset.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `clear-basis-469515-u9.silver_dataset.quality_checks_cpt_codes`;
